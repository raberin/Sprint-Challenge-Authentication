...
1. What is the purpose of using _sessions_?
   A session is used to persist authentication information across requrests. That way one doesnt have to input credentials after a request is made.

1. What does bcrypt do to help us store passwords in a secure manner.
   bcrypt hashes a password/string into a long string. This string hashing is one way only and cannot be reversed to retrieve the string. When someone registers the hashed password is sent to the db instead of the actual string

1. What does bcrypt do to slow down attackers?
   Not only does bcrypt hashes passwords but it also adds time as a variable into the security algorithm to create a key derivation function.

1. What are the three parts of the JSON Web Token?
   A JWT token has a header, payload and signature.
